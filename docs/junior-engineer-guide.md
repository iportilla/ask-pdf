...
- **Increase the Ollama request timeout further:** the `client_kwargs={"timeout": 300}` values in `build_llm_and_embeddings()` (5 minutes) — bump this if you regularly hit timeouts embedding very large files on slow/CPU-only hardware. The separate `check_ollama_reachable(..., timeout=10)` fast-fail probe can stay short regardless.
...
- **Large `.xlsx` uploads can still time out against Ollama even once networking is fixed** — this is a third, distinct cause from the two above. `OllamaEmbeddings.embed_documents()` sends every chunk of the file in **one single batched HTTP request** (confirmed by reading `langchain_ollama`'s source), and embedding a few hundred chunks from a large spreadsheet on CPU-only hardware can legitimately take minutes; a small PDF has far fewer chunks and finishes fast, which is why this only shows up on Excel. `build_llm_and_embeddings()` addresses this with two separate timeouts: `check_ollama_reachable()` does a short (~10s) probe first so a truly unreachable server still fails fast, while the real `OllamaLLM`/`OllamaEmbeddings` clients get a much longer `client_kwargs={"timeout": 300}` for the actual work. `main()` also wraps the embed/generate steps in `st.spinner(...)` so a legitimately slow local run doesn't look hung. `describe_provider_error()` tells this case apart from a real networking failure via `isinstance(exc, httpx.ReadTimeout)` (probe succeeded, but the real request didn't finish in time).
